...
- `gemini` — read-only Gemini batched-RPC client. It bootstraps the page's `SNlM0e`/`cfb2h`/`FdrFJe` values, uses `MaZiqc` for conversation pages (including the separate pinned set), and uses `hNvQHb` for 100-turn pages. **Always exhaust the opaque cursor and reject repeated cursors**; a single response page is not a complete conversation. Gemini records are positional arrays: decode known fields defensively, normalize human/assistant `message`, IDs, parent links, `createTime`, and attachments for the shared exporter, and retain raw turns/pages for `raw-json`. Missing bootstrap tokens map to `grok.ErrAuthExpired` so CLI/web behavior stays consistent.
- Gemini can return an HTTP-200 error envelope after its cached page bootstrap tokens rotate. A missing `wrb.fr` payload invalidates `SNlM0e`/`cfb2h`/`FdrFJe` and retries the read-only RPC once; preserve this behavior so users are not forced to re-sync a still-valid browser session.
- `exporter` — formats `markdown`, normalized `json`, archival `raw-json`, and `zip`. Markdown: YAML frontmatter, responses stably sorted by `createTime`, empty bodies skipped. Normalized JSON uses named provider-neutral conversation/response fields, sorts by `createTime`, and omits raw provider blobs; raw JSON preserves upstream shapes and uses `.raw.json`. ZIP contains Markdown, normalized JSON, and `media/`. Media downloads dedupe by URL, cap at 64 MB, and rewrite URLs in the Markdown body to local `media/` paths. `SafeName` sanitizes titles for the filesystem (path-escape tests exist — keep them passing). Progress is reported via callbacks with phases `loading`/`responses`/`writing`.
